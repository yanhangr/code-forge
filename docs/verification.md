...
| 验证 | 结果 | 能力边界 |
| --- | --- | --- |
| 核心、适配与迁移测试 | 55 项通过 | 状态不变量、幂等、UserBinding、Session/Message 原子忙碌拒绝、用户默认/显式多 Skill 路径、Workspace lease/revision、SQLite v1→v4 迁移 |
| PostgreSQL 语法解析 | pglast 8.4 解析 197 条语句通过 | 没有在 PostgreSQL 实例执行，不证明迁移/FK/真实并发运行通过 |
| JSON Schema | 生成 66 个组件 schema，一致性检查通过 | 不等于所有生产客户端已完成兼容验证 |
...
| 浏览器 Session/Message 主流程 | Playwright 实测新建 Session、发送 Python Message、接收完整 SSE、显示工具/Skill 轨迹和 Workspace 文件；桌面与 390px 移动视口截图无重叠 | 仅证明 trusted logical 模式，不证明用户容器/执行沙箱；首次默认 Runtime 端口探测和 favicon 仍有无害 404 |
